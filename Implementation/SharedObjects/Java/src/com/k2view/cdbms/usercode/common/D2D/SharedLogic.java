...
public class SharedLogic {
    ...
    public static Object fnD2DCompare(Object row, String luName, String Source_Transformation_Function_Name, String Target_Transformation_Function_Name, String customizedKeyComparison, String source_columns_to_Ignore_null, String target_columns_to_Ignore_null) throws Exception {
        ...
        sourceMap.forEach((key, value) -> {
            ...
            if ((value == null && targetValue == null) || (value != null && value.equals(targetValue)) || (value != null && targetValue == null && tctin.contains(key.toUpperCase())) || (value == null && targetValue != null && sctin.contains(key))) {
    ...
    @out(name = "result", type = Object.class, desc = "")
    @out(name = "customizedKeyComparison", type = String.class, desc = "")
    public static Object fnD2Dv2Compare(Object row, String luName, String ColumnMapping, String Source_Transformation_Function_Name, String Target_Transformation_Function_Name, String customizedKeyComparison, String source_columns_to_Ignore_null, String target_columns_to_Ignore_null) throws Exception {
        Map<String, String> sourceMapTransformationFunction = getFunction2Column(Source_Transformation_Function_Name);
        Map<String, String> targetMapTransformationFunction = getFunction2Column(Target_Transformation_Function_Name);
        //Test add
        ColumnMapping = ColumnMapping.toUpperCase();
        Map<String, String> resultMap = new HashMap<>();

        // Split by commas first to get key-value pairs
        String[] pairs = ColumnMapping.split("\\|");

        for (String pair : pairs) {
            String[] keyValue = pair.split(":"); // Escape | since it's a regex special character
        ...
        sourceMap.forEach((key, value) -> {
            ...
            if(resultMap.containsKey(key)){
                columnResult.put("target_value",targetMap.get(resultMap.get(key)));
            }
            else{
                columnResult.put("target_value", targetMap.get(key));
            }
            ...
            //Second test ADD loop
            if(resultMap.containsKey(key)){
                columnResult.put("target_column_orig_value",targetMap.get(resultMap.get(key)+ "_k2orig"));
            }
            else{
                columnResult.put("target_column_orig_value", targetMap.get(key + "_k2orig"));
            }
    ...
    @out(name = "result", type = Object.class, desc = "")
    @out(name = "customizedKeyComparison", type = String.class, desc = "")
    public static Object fnF2FCompareSourceNTarget(Object row, String ColumnMapping, String customizedKeyComparison,
        ...
        for (Map.Entry<String, Object> entryRow : ioSimpleRow.entrySet()) {
            ...
            if (entryRow.getKey().startsWith(sourceEnv)) {
                columnName = entryRow.getKey().replace(sourceEnv + "_", "");
                sourceMap.put(columnName + "_k2orig", columnValue != null ? columnValue.toString() : null);
                ...
                columnName = entryRow.getKey().replace(targetEnv + "_", "");
                targetMap.put(columnName + "_k2orig", columnValue != null ? columnValue.toString() : null);
    ...
    public static String fnD2DGetSource(String luName, String sourceTableName, String customizedKeyComparison, String source_Transformation_Function_Name, String excludedColumnsNames) throws Exception {
        List<String> excludedColumnsNamesList = new ArrayList<>();
        for (String columnToIgnore : excludedColumnsNames.split(DELIMITTER)) {
            excludedColumnsNamesList.add(columnToIgnore.toUpperCase());
        }
    ...
    public static String fnD2DGetTarget(String luName, String targetTableName, String target_Transformation_Function_Name, String customizedKeyComparison, String excludedColumnsNames) throws Exception {
        List<String> excludedColumnsNamesList = new ArrayList<>();
        for (String columnToIgnore : excludedColumnsNames.split(DELIMITTER)) {
            excludedColumnsNamesList.add(columnToIgnore.toUpperCase());
        }
    ...
    private static Object getTransformedValue(String customFunctionName, LUType luType, Object columnValue) throws ReflectiveOperationException, InterruptedException, SQLException {
        ...
        if (customFunctionName != null) {
            Db.Row row = fabric().fetch(String.format("Broadway %s.%s value=?", luName, customFunctionName), columnValue).firstRow();
    ...
    public static List<Map<String, String>> fnGetTablePopulationDetails(String luName, String tableName) throws Exception {
        List<Map<String, String>> tablePopDetails = new ArrayList<>();
        List<TablePopulationObject> popObj = ((TableObject) (LUTypeFactoryImpl.getInstance().getTypeByName(luName)).ludbObjects.get(tableName)).getAllTablePopulationObjects();
        popObj.forEach(obj -> {
	...
	@out(name = "result", type = Object.class, desc = "")
	@out(name = "customizedKeyComparison", type = String.class, desc = "")
	public static Object fnF2Fv2Compare(Object row, String customizedKeyComparison, String source_columns_to_Ignore_null, String target_columns_to_Ignore_null, String sourceEnv, String targetEnv, String Source_Transformation_Function_Name, String Target_Transformation_Function_Name) throws Exception {
		Map<String, String> sourceMapTransformationFunction = getFunction2Column(Source_Transformation_Function_Name);
		Map<String, String> targetMapTransformationFunction = getFunction2Column(Target_Transformation_Function_Name);
	...
	public static Object convertTableMetadata(Object inputData) throws Exception {
		...
		if (itr != null) {
		    ...
		    for (Map<String, Object> entry : input) {
		        ...
		        tableData.put("Customized_Key_Comparison", tableData.getOrDefault("Customized_Key_Comparison", "") + (isPk ? (tableData.getOrDefault("Customized_Key_Comparison", "").equals("") ? column : "|" + column) : ""));
	...
	public static Object filterTableMetadata(List<Map<String,Object>> tableList) throws Exception {
		List<Object> filteredTableList = tableList.stream()
                  ...
                  .filter(entry -> entry.get("Target_Table_Name") != null && !entry.get("Target_Table_Name").toString().isEmpty())
                  .filter(entry -> entry.get("Source_Table_Name") != null && !entry.get("Source_Table_Name").toString().isEmpty())
